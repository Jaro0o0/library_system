...
function Header() {
    ...
    useEffect(()=>{
        ...
        const res  = await fetch(`http://localhost:5000/search/Books/recomended?userId=${userId}&count=10`,{
    ...
    const searchBooksHandler = async (
        ...
        const res = await fetch(
            `http://localhost:5000/search/Books/search-books?title=${encodeURIComponent(input)}`,
            {
                method: "GET",
                headers: { "Content-Type": "application/json" },
            }
    ...
    useEffect(  () => {
        const getReccomendation =   async  () => {
                ...
                const res = await fetch(`http://localhost:5000/search/Books/recomended?userName=${ userName }`,{
                method: 'GET',
                });
    ...
    <>
        ...
        {/* Search_Menu */}
            ...
            <div
                ...
                className={`fixed left-0 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 z-[99] py-8 shadow-xl transition-transform duration-500 font-[400] ${searchOpen ? "translate-y-0" : "-translate-y-full"}`}
            >
                <Container>
