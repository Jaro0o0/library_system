...
function CategoryPageSwiper() {
    ...
    useEffect(() => {
        (async () => {
            setLoading(true);
            try {
                const res = await fetch(`http://localhost:5000/search/Books/category/${encodeURIComponent(genre)}`);
                if (!res.ok) {
            ...
            {loading ? (
                ...
                >
                    {books.map((book) => {
                        ...
                        const cover = `http://localhost:5000/images/Images?title=${encodeURIComponent(volume.title)}`;
                        return (
