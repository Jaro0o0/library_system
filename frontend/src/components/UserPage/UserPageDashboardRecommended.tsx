        ...
        const getData = async () => {
            ...
            try {
                const res = await fetch(
                    `http://localhost:5000/search/Books/recomended?userName=${encodeURIComponent(userName)}`,
                    {
                        method: "GET",
                        headers: {
                    ...
                    {data.length === 0 ? (
                        ...
                        data.map((item, index) => (
                            ...
                            >
                                ...
                                <div>
                                    <img  src={`http://localhost:5000/images/Images?title=${encodeURIComponent(item.tytul)  }`}  className="rounded-2xl object-cover w-[150px] h-[150px]" alt="product-img"/>
                                    </div>
                                <div>
