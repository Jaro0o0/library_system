    ...
    return ( 
        <>
            ...
            <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {data.endDate > nowDatae ? <p>you don't have active rents yet  </p> :
                    ...
                    data.map((item) => (
                        <div  key={item.id} className='flex flex-col gap-4  bg-white rounded-2xl shadow-md hover:shadow-md transition-shadow duration-300'>
                            <div>
                                <img src ={`http://localhost:5110/images/Images?title=${encodeURIComponent(item.book.tytul)  }`} alt='book-img' className=' max-h-[200px] w-full object-cover  rounded-t-lg'/> 
                            </div>
                            <div className='p-5'>
